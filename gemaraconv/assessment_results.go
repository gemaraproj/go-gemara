...
func EvaluationLogToOSCALAssessmentResults(log gemara.EvaluationLog, opts ...EvalOption) (oscal.AssessmentResults, error) {
	...
	metadata := createAssessmentResultsMetadata(log)

	result, err := evaluationLogToResult(log, options.catalog)
	if err != nil {
...
func evaluationLogToResult(log gemara.EvaluationLog, catalog *gemara.ControlCatalog) (oscal.Result, error) {
	now := time.Now()
...
func createAssessmentResultsMetadata(log gemara.EvaluationLog) oscal.Metadata {
