...
.PHONY: all tidy fmtcheck fmt vet lint test testcov race coverage-check build install generate version-stamp ci-local clean help
...
# Write version.go from SPECVERSION so the exported SchemaVersion cannot drift
# from the schema the types are actually generated from.
version-stamp:
	@echo " > Stamping version.go with $(SPECVERSION)"
	@printf '%s\n' \
	  ...
	  'const SchemaVersion = "$(SPECVERSION)"' \
	  > version.go
	@gofmt -w version.go
...
help:
	...
	@echo "  generate       - generate Go types (stamps version.go from SPECVERSION)"
	@echo "  version-stamp  - rewrite version.go from SPECVERSION"
